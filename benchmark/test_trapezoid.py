...
"""Run tests on Trapezoid against Lee-Preparata."""
import argparse
import os.path
import signal
...
def test_algorithms(path, polygon, s, t, pictures):
    ...
    try:
        ...
        trapezoid_path = list(trapezoid_shortest_path(polygon, Point(*s.tuple()), Point(*t.tuple())))
    except TimeoutError:
        print('\nTrapezoid calculation took longer than {0}s!'.format(timeout))
        return
        ...
        with open(file_name + '.exception', 'w') as f:
            print('Trapezoid-Exception', file=f)
            print('{0} -> {1}\n'.format(s, t), file=f)
    ...
    if trapezoid_path != lee_preparata_path:
        if pictures:
            save_polygon(file_name, polygon, trapezoid_path, lee_preparata_path, s.index, t.index)
        ...
        for path_ix in range(len(trapezoid_path) - 1):
            for edge_ix in range(len(polygon)):
                if polygon.edge(edge_ix).properly_intersects(
                    ...
                    if pictures:
                        save_polygon(file_name, polygon, trapezoid_path, lee_preparata_path, s, t)
                    print('\nFailed: {path}, {s} -> {t}'.format(path=name, s=s.index, t=t.index))
                    error = True
                    break
            if error:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test trapezoid against Lee-Preparata.')
